...
public class ModuleAttributeTest01 {
    ...
    enum Kind {
        ...
        NAME("module M; package P; class C { }", "module M { }"),
        NAME_AND_VERSION("module M; package P; class C { }", "module M@1.0 { }");
    ...
    void run() throws Exception {
        for (Kind k: Kind.values()) {
            try {
                System.err.println("Test " + k);
                test(k.classBody, k.moduleInfoBody);
            } catch (Throwable t) {
                t.printStackTrace();
                errors++;
            }
    ...
    void test(String classBody, String moduleBody) throws Exception {
        count++;
        reset();
        List<File> files = new ArrayList<File>();
        addFile(files, createFile("C.java", classBody));
        addFile(files, createFile("module-info.java", moduleBody));
        compile(files);
        String moduleId = getModuleId(moduleBody);
        checkModuleAttribute("P/C.class", moduleId);
        if (moduleBody != null)
